...
//
//////////////////////////////////////////////////////
CDLPFileInfo::CDLPFileInfo(CDebugInfo *potDebug)
{
	poDebug = potDebug;
	poFileInfo = new CFileInfo;
...
//
//////////////////////////////////////////////////////
unsigned long CDLPFileInfo::nGetUID(void)
{
	return nUID;
}
...
//
//////////////////////////////////////////////////////
void CDLPFileInfo::vSetInfo(unsigned long ntUID,
					char *pFileName,
					unsigned long ntSize,
					char *pDate,		
					char *pVersion,		
					char *pPassword,	
					char *pCRC
...
{
					nUID = ntUID;
					poFileName->Format("%s",pFileName);	
					nSize = ntSize;
					poDate->Format("%s",pDate);		
					poVersion->Format("%s",pVersion);		
					poPassword->Format("%s",pPassword);	
					poCRC->Format("%s",pCRC);
...
//
//////////////////////////////////////////////////////
CStr * CDLP_TIME::poGetDate(void)
{
	CStr *poStr = new CStr;
	poStr->poGetDateTime(&oSystemTime);
...
//
//////////////////////////////////////////////////////
t64 CDLP_TIME::FindTimeTBase(void)
{
...
//
//////////////////////////////////////////////////////
void CDLP_TIME::vGetCurrentTime(void)
{
	GetSystemTime(&oSystemTime);
}
...
//
//////////////////////////////////////////////////////
void CDLP_TIME::vSetTime(const FILETIME *poFileTime)
{
	FileTimeToSystemTime(poFileTime,&oSystemTime);
}
...
{
	poFileName = new CStr;
	poFileDrive = new CStr;
	poFileDir = new CStr;
	poFileTitle = new CStr;
	poFileExt = new CStr;
	poFileRoot = new CStr;
	poFilePath = new CStr;

   m_strFilePath.Format("");
   m_dwChecksum = 0;
   m_dwCRC = 0xFFFFFFFF;
   ...
   m_timCreation.vGetCurrentTime();
   m_timLastAccess.vGetCurrentTime();
   m_timLastWrite.vGetCurrentTime();
...
void CFileInfo::Create(const WIN32_FIND_DATA* pwfd, const char * strFilePath, LPARAM lParam) {

	m_strFilePath.Format("%s\\%s",strFilePath,pwfd->cFileName);
   ...
   m_timCreation.vSetTime(&pwfd->ftCreationTime);
   m_timLastAccess.vSetTime(&pwfd->ftLastAccessTime);
   m_timLastWrite.vSetTime(&pwfd->ftLastWriteTime);

   m_lParam = lParam;
}

void CFileInfo::Create(const char * strFilePath, LPARAM lParam) 
{
   WIN32_FIND_DATA wfd;
   HANDLE hFind;
   int nBarPos;
   ...
   {
	   CStr oStr;
	   oStr.Format("%s",strFilePath);
       nBarPos = oStr.ReverseFind(TCHAR('\\'));
	  
	  oStr.Format("%s",strFilePath);
	  CStr oResult;
	  oStr.GetLeft(nBarPos+1,&oResult);
	 
	  
	  Create(&wfd,oResult.GetString(), lParam);
      FindClose(hFind);
   }
...
DWORD CFileInfo::GetChecksum(const ULONGLONG uhUpto, const BOOL bRecalc, 
   ...
   if (m_uhChecksumBytes < uhCount) {
      // Not all bytes summed
	   hf = CreateFile(m_strFilePath.GetString(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 
...
char * CFileInfo::pGetFileDrive(void) 
{
   TCHAR szDrive[_MAX_DRIVE];
   _tsplitpath(m_strFilePath.GetString(), szDrive, NULL, NULL, NULL);
   poFileDrive->Format("%s",szDrive);
   return poFileDrive->GetString();;
}
 
char * CFileInfo::pGetFileDir(void) 
{ 
   TCHAR szDir[_MAX_DIR];
   _tsplitpath(m_strFilePath.GetString(), NULL, szDir,NULL, NULL);
   poFileDir->Format("%s",szDir);
   return poFileDir->GetString();
}

char * CFileInfo::pGetFileTitle(void)
{
   TCHAR szName[_MAX_FNAME];
   _tsplitpath(m_strFilePath.GetString(), NULL, NULL, szName, NULL);
   poFileTitle->Format("%s",szName);
   return poFileTitle->GetString();
}

char * CFileInfo::pGetFileExt(void)
{
   TCHAR szExt[_MAX_EXT];
   _tsplitpath(m_strFilePath.GetString(), NULL, NULL, NULL, szExt);
   poFileExt->Format("%s",szExt);
   return poFileExt->GetString();
}

char * CFileInfo::pGetFileName(void)
{ 
   poFileName->Format("%s.%s",pGetFileTitle(),pGetFileExt());
   return poFileName->GetString();
}

char * CFileInfo::pGetFileRoot(void)
{ 
	poFileRoot->Format("%s\\%s",pGetFileDrive(),pGetFileDir());
	return poFileRoot->GetString(); 
};

char * CFileInfo::pGetFilePath(void) 
{
	poFilePath->Format("%s",m_strFilePath.GetString());
	return poFilePath->GetString(); 
}


CFileInfoArray::CFileInfoArray(CDebugInfo *potDebug) 
{
	poDebug = potDebug;
	poArray = new DLP_CPtrArray();
}

CFileInfoArray::~CFileInfoArray() 
{
	for(int i=0; i<poArray->nGetSize(); i++)
	{
		CFileInfo *poFile = (CFileInfo *)poArray->pGetAt(i);	
		delete poFile;
	}	
...
int CFileInfoArray::AddFileInfo(CFileInfo *finf, LPARAM lAddParam) 
{
   ...
   switch (lAddParam) 
   {
      case AP_SORTASCENDING | AP_SORTBYSIZE:
		  {
			for(i=0; i<poArray->nGetSize(); i++)
			{
				CFileInfo *poFile = (CFileInfo *)poArray->pGetAt(i);

				if(i<poArray->nGetSize() && ((poFile->GetLength64()<finf->GetLength64())))
				{
					break;
				}
			}
      ...
      case AP_SORTASCENDING | AP_SORTBYNAME:
		  {
			for(i=0; i<poArray->nGetSize(); i++)
			{
				...
				size_t nLen0 = strlen(poFile->pGetFilePath());
				size_t nLen1 = strlen(finf->pGetFilePath());
				if(i<poArray->nGetSize() && ((nLen0<nLen1)))
				{
					break;
				}
			}
   ...
   // Insert it
   poArray->vInsertAt(i,(void *)finf);
   return i;
}

int CFileInfoArray::AddDir(const char * strDirName, const char * strMask, const BOOL bRecurse,
   ...
   CStr oStr;
   oStr.Format("%s\\%s",strDirName,"*");

   //strText = strDirName + _T("*");
   strText.Format("%s",oStr.GetString());
   
   // Iterate through dirs
   hFind = FindFirstFile(strText.GetString(), &wfd);
   if (hFind != INVALID_HANDLE_VALUE) {
      do {
         if ((strcmp(wfd.cFileName,".")) && (strcmp(wfd.cFileName,"..")) && 
             (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
            
				 oStr.Format("%s\\%s",strDirName,wfd.cFileName);
				 strText.Format("%s",oStr.GetString());
				 //strText = strDirName + wfd.cFileName;

            if (bIncludeDirs) 
			{
				CFileInfo *finf = new CFileInfo;
            ...
            if (bRecurse)
			{
               // Recurse Dirs
				...
				oStr.Format("%s\\",strText.GetString());
				nCount += AddDir(oStr.GetString(), strMask, bRecurse, lAddParam, 
   ...
   {
	   oStr.Format("%s\\%s",strDirName,strMask);
	   strText.Format("%s",oStr.GetString());
      ...
      if (hFind != INVALID_HANDLE_VALUE) {
         do {
			...
			{
				strText.Format("%s\\%s",strDirName,wfd.cFileName);
				CFileInfo *finf = new CFileInfo;
               finf->Create(&wfd, strDirName);
               AddFileInfo(finf, lAddParam);
               nCount++;
...
int CFileInfoArray::AddFile(const char * strFilePath, LPARAM lAddParam) {
   CFileInfo *finf = new CFileInfo;
   finf->Create(strFilePath);
   int nVal = AddFileInfo(finf, lAddParam);
   return nVal;
}
