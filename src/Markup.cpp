...
int CMarkup::x_ParseElem( int iPosParent )
{
	...
	while ( csName.IsEmpty() )
	{
		...
		if ( x_FindToken( token ) )
		{
			...
			char cFirstChar = m_csDoc->GetAt(token.nL);
	...
	//{
		...
		//			return x_ParseError( "Expecting end tag of element %s", csName );
		//
		//		else if ( nTokenCount == 2 && ! token.Match(csName) )
		//			return x_ParseError( "End tag does not correspond to %s", csName );
		...
		//	return x_ParseError( "End tag not completed for element %s", csName );
...
CStr * CMarkup::x_GetToken( const CMarkup::TokenPos& token ) const
{
...
CStr * CMarkup::x_GetData( int iPos ) const
{
	...
	if ( ! m_aPos[iPos].iElemChild && ! m_aPos[iPos].IsEmptyElement() )
	{
		...
		if ( x_FindAny( szDoc, nChar ) && szDoc[nChar] == '<'
				...
				&& _tcsncmp( &szDoc[nChar], "<![CDATA[", 9 ) == 0 )
		{
			nChar += 9;
